...
export function generateSupplierId() {
    ...
    let id = parseInt(localStorage.getItem(KEYS.supplierId))
    localStorage.setItem(KEYS.supplierId, (++id).toString())
    return id;
}

export function getAllSuppliers() {
    if (localStorage.getItem(KEYS.supplier) == null)
        localStorage.setItem(KEYS.supplier, JSON.stringify([]))
    let supplier = JSON.parse(localStorage.getItem(KEYS.supplier));
    //map departmentID to department title
    let departments = getDepartmentCollection();
    return supplier.map(x => ({
        ...x,
